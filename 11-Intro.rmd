...
CSR is often characterized as a **homogeneous Poisson process**, where events are independently and uniformly distributed across the study region. CSR provides a benchmark against which we can compare observed patterns. Deviations from CSR suggest the presence of non-random spatial structure--either in the form of large-scale variation or local interaction. 

While deviations from CSR suggest non-random spatial structure—either in the form of large-scale variation (first-order effects) or local interaction (second-order effects)—it is important to note that **distinguishing between these two types of effects from a single observed pattern can be challenging**.

### First-Order Effects: Spatial Trend and Intensity

First-order properties describe how the **event intensity** (the average rate or expected number of events) varies across space. For continuous field data, these are often referred to as spatial trends. These effects reflect broad, underlying factors that influence the distribution of events.
...
Second-order properties describe local dependencies--how the presence or value of one observation influences others nearby. These effects are often referred to as **spatial autocorrelation**.

**Examples:**
- Trees clustering due to seed dispersal.
- High crime rates spreading to adjacent neighborhoods due to local dynamics.

Second-order properties describe local dependencies (how the presence or value of one observation influences others nearby). For point data, these are often called interactions between events, while for continuous field data, they are referred to as spatial autocorrelation. Analyzing second-order properties typically involves assessing patterns of interaction **beyond any underlying spatial trend or varying intensity**.
...
First-order and second-order effects offer complementary insights into spatial patterns. An observed cluster might be due to a spatial trend (first-order), local interaction (second-order), or both. However, **it is often difficult to distinguish these effects in practice from a single observed pattern**. Failing to distinguish between these can lead to incorrect conclusions about the underlying process, a common pitfall in spatial analysis. Based on a single realization, there is no mathematical way to distinguish between a process of independent events generated under a heterogeneous intensity and a process of dependent events generated under a homogeneous intensity.
...
We learned in [chapter 2](https://mgimond.github.io/Spatial/chp02_0.html#object-vs.-field) that spatial data can be broadly categorized into two types: **object-based**, which treat the world as discrete entities located in space (e.g., points for crime locations or tree positions, or polygons for administrative boundaries), and **field-based**, which represent continuous phenomena measured across space (such as temperature, elevation, or precipitation). These distinct representations influence how spatial patterns are analyzed.
...
The following list of resources is designed for readers new to spatial analysis with an introductory background in statistics, ordered by increasing complexity.:
...
- **Chapter 16: Interpolation**
   Explore techniques such as inverse distance weighting (IDW), trend surfaces, and kriging to interpolate values to estimate unknown values at unsampled locations using spatial data.
