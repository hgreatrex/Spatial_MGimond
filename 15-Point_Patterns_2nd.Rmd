...
In the sections that follow, we introduce three distance-based techniques:
...
```{r f11-ppp-dist, echo=FALSE, fig.cap="A subset of distances between each point and its closest point. For example, the point closest to point 3 is point 4 which is 16,616 meters away.", results="asis", message = FALSE}
library(ggplot2)
library(sf)
library(spatstat)
library(gridExtra)
...
S  <- st_read("./Data/Walmarts.shp", quiet = TRUE)
P <- as.ppp(S)
marks(P) <- NULL
...
- The K function, as presented here, assumes a homogeneous underlying first-order process given that $\hat{\lambda}$ is assumed uniform for all locations within the study extent.
...
where $w_{ij}$ is the isotropic corrected weight that is the proportion of the circumference of the circle centered at $i$ with radius $d$ that lies within the study area $A$. Note that the proportion is that of the circle's **perimeter length** and not the circle's area.
...
```{r f11-L, echo=FALSE, fig.cap="L function rendering of the K function. Both the edge corrected (red line) and standard L functions (black line) are shown. The theoretical L function under CSR is shown as a horizontal line centered on 0. "}
